...
/*!
  ...
  V2 tweaks made by Vampix

  V3 tweaks made by Mich
*/
/**************************************************************************/
class Knot : public Pattern {
  public:
    ...
    motionParameter nextTarget(unsigned int index) {
        ...
        // Fancy equation to calculate the pause in milliseconds using the current speed as an input
        ...
        if (_isStillDelayed() == false) {
            if (index % 5 == 1) {
                ...
                // 70% of the stroke looks good visually
...
/*!
  ...
  are unwanted behavior in this context, so it's bounded to a min of 0.5
